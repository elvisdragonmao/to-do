...
- List View：列出所有 sprint 與 Backlog 項目，依分類分表，八個欄位皆可升冪／降冪排序。
- Backlog：只顯示尚未排入 sprint、沒有預計工作日期的項目，依分類整理；可直接拖進目前 sprint 的任意狀態與順序。
...
- 整張卡片拖曳：拖曳期間 sprint pager 會鎖在目前週；每個插入位置都有即時空位預覽，放開後直接落位、不播放回彈動畫。也能拖到小日曆日期安排工作日；日曆下方的垃圾桶只在拖曳時出現。
- 卡片原位編輯：標題、描述、分類、時數、預計日期與 Deadline 直接轉為 input/select，不開啟放大 dialog；緊急程度直接點最右側旗子調整，Kanban 不重複顯示狀態，排程歷史與完成日期也不佔卡片空間。空描述不佔版面，從標題按 `Enter` 進入描述、按 `Cmd`/`Ctrl` + `Enter` 一次儲存文字欄位。
